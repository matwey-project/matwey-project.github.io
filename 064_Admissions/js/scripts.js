...
$(document).ready(function () {
	...
	if ($('.tab-category').length) {
		$('.tab-category').mixItUp({
			selectors: {
				filter: '.filter'
			}
		});
	}
	if ($('.our-tutors--wrapper').length) {
		$('.our-tutors--wrapper').mixItUp({
			selectors: {
				filter: '.filter-teatcher'
			}
		});
	}
...
var formFunction = function () {
	var stepCount = 0,
		stepContainer = $('.form-steps--block').length - 1,
		stepTop = $('.step-form--wrapper .step-form--items').length - 1,
		next = $('.step-next'),
		prev = $('.step-prev');

	// $('.form-steps--block').each(function(i, items){
	// 	// stepContainer++;
	// });
	...
	next.on('click', function (e) {
		e.preventDefault();
		if (stepCount >= stepContainer - 1) {
			$('#submit--form').show();
			next.hide();
		}
		if (stepCount >= stepContainer) {
			stepCount = stepContainer;

			console.log("stepCount " + stepCount);
			stepTopFunc(stepCount);
		} else {
			stepCount++;
			// $($('.form-steps--block')[stepCount]).addClass('step-active');
			stepGo(stepCount);
			stepTopFunc(stepCount);
		}
	});
	prev.on('click', function (e) {
		...
		if (stepCount <= 0) {
			...
			$('#submit--form').hide();
			stepGo(stepCount);
			stepTopFuncD(stepCount);
		}
	});
	function stepGo(counter) {
		$('.form-steps--block').removeClass('step-active');
		$($('.form-steps--block')[counter]).addClass('step-active');
		// $($('.form-steps--block')[stepCount]).addClass('step-active');
	}
	function stepTopFunc(counter) {
		$($('.step-form--wrapper .step-form--items')[counter]).addClass('active');
	}
	function stepTopFuncD(counter) {
		$($('.step-form--wrapper .step-form--items')[counter + 1]).removeClass('active');
	}
};
formFunction();
...
var menuOpen = function () {
	$(".burger-menu").click(function (e) {
		...
		$(".section-school--step").toggleClass("remove-index");
		$("#overlayer").toggleClass("is-active");
		$(".burger-menu").toggleClass("is-active");
	});
	$(document).mouseup(function (e) {
		var container = $(".header-head");
		if (container.has(e.target).length === 0) {
			$(".header-menu--wrapper").removeClass("is-active");
			$(".section-school--step").removeClass("remove-index");
...
$('.header-menu .sub-menu').each(function() {
	...
	$(this).parent('li').children('a').after('<span class="list-open"></span>')
	
});
$('.list-open').click(function(){
	console.log($(this).next('.sub-menu').slideToggle());
})

// var btn1 = document.querySelector("#clickB1");
// var btn2 = document.querySelector("#clickB2");

// function checkBind() {
// 	console.log(this.innerHTML);
// }
// var funClick = checkBind.bind(btn2);
// btn1.addEventListener("click", checkBind);
// btn2.addEventListener("click", funClick);