...
// upload/filepond-plugin-file-encode.min.js
// upload/filepond-plugin-file-validate-size.min.js
// upload/filepond-plugin-image-exif-orientation.min.js
// upload/filepond-plugin-image-preview.min.js
// upload/filepond.min.js
...
$('#curent-year').html(fullDate);

// Form Settings

var countryCheck = function(){

  // var input = document.querySelector(".countryCheck");
  // var iti = window.intlTelInput(input, {
  //   utilsScript: "https://cdn.jsdelivr.net/npm/intl-tel-input@16.0.3/build/js/utils.js",
  // });
  // window.iti = iti;

  var input = $(".countryCheck");
  input.intlTelInput({
  ...
  input.on("change", function() {
    ...
    console.log(input.intlTelInput("setCountry", "gb"))
  });

$('.country').click(function(){
  console.log($(".countryCheck").intlTelInput("getSelectedCountryData").dialCode);
});
$(document).on("click", function(){
  if($('.intl-tel-input')){
    $('.phone-word input').val("+"+input.intlTelInput("getSelectedCountryData").dialCode)
    input.val(input.intlTelInput("getSelectedCountryData").name)
...
// });
  ...
  $(".countryCheck").change(function(){
    $(".countryCheck").val(input.intlTelInput("getNumber"));
  });

  $('.countryCheck-wrap').each(function(){
    var valCode = $(this).parent('.input-items').next().find('.phone-items .phone-word input');
    var valInput = $(this).find('.countryCheck');
    var phoneMain = $(this).parent('.input-items').next().find('.phone-items input.phone-main');
    // var phoneMain = $('.phone-main');
    var telHiddeInput = $(this).parent('.input-items').next().find('.phone-items input.phone-val');
    // var telHiddeInput = $('.phone-val');
    var resultPhone, onInp = "";
    var codeCount = "";

    $(this).find('.country-list li').on('click', function(){
      var nameCount = $(this).find('.country-name').text();
      codeCount = $(this).find('.dial-code').text();
      // var telSumm = codeCount 
      valInput.val(nameCount);
      valCode.val(codeCount);
      
      
      
    });
    $('.intl-tel-input').find('.country-list li').on('click', function(){
      var nameCount = $(this).find('.country-name').text();
      codeCount = $(this).find('.dial-code').text();
      // var telSumm = codeCount 
      valInput.val(nameCount);
    ...
    phoneMain.on('input', function(){
      onInp = $(this).val();
      resultPhone = $('.phone-word input').val() + "" + onInp;
      console.log(resultPhone)
      telHiddeInput.val(resultPhone);
    });
    // console.log(codeCount+ "PPP")
    // resultPhone = codeCount + "" + onInp;
    // console.log(resultPhone)
    // telHiddeInput.val(codeCount);
    // $(this).find('.country-list .country-name').text();
  });
...
$('#get-touch').on('click', function(e){
  e.preventDefault();
  $('body').addClass('no-overflow');
  $('.form-hide, .overlay-bg, .form-perspective, .icon-close-form').addClass('is-active');
});
$('.icon-close-form, .close-fill, .back-done').on('click', function(){
  $('body').removeClass('no-overflow');
  $('.form-hide, .overlay-bg, .form-perspective, .icon-close-form, .succesfull-post').removeClass('is-active');
});

$('.phone-mask').mask('999-999-9999', {autoclear: true}, {"placeholder": ""});
$("#textarea-send").keyup(function(){
    el = $(this);
    if(el.val().length >= 500){
        el.val( el.val().substr(0, 500) );
    } else {
        $("#charNum").text(el.val().length+'/500');
    }
});
$(document).ready(function() {

  // function initCustomScrollbar() {
  //   var $selector = $(".form-hide");
...
//   for(var i=0;i<document.querySelectorAll('input[type="file"]').length; i++){
//     console.log(this);
//   }
// };
// fileLoad();
...
// });
  ...
  //     {
  //       labelIdle: '<i class="upload-icon"></i> upload',
  //     }
  //   );
  // FilePond.create(
  //   document.querySelector('.filepond-contact'),
  //     {
  //       labelIdle: '<i class="upload-icon"></i> upload',
  //     }
  //   );
  ...
  var checkForm = function(){
    $('.input-items').each(function (index, value) {
  ...
  $('#send-form').on('click', function(){
    ...
    payError($('input[name="email"]'))

    // $('label.checkbox-default').each(function (index, value) {
    $(this).parent('form').find('label.checkbox-default').each(function (index, value) {

      $(this).on('click', function(){
        $(this).find('input[type="checkbox"]').removeClass('checked-error');
        $('#send-form').removeAttr("disabled");
      });
      
      if($(this).prop('checked', false)) {
        $(this).find('input[type="checkbox"]').addClass('checked-error');
      } else {
        $(this).find('input[type="checkbox"]').removeClass('checked-error');
      }
    });

     $(this).parent('form').find('.input-items').each(function (index, value) {

      if($(this).find('input[type="text"]').val()) {
        $(this).find('input[type="text"]').removeClass('input-error');
      } else {
  ...
  function payError(elem){
    if(!elem.val()) {
      elem.addClass('inpur-error')
      $('#send-form').attr("disabled", true);
    } else {
      elem.removeClass('inpur-error')
      $('#send-form').removeAttr("disabled");
    }
...
$(".js-select2").each(function(){
  $(this).select2({
    ...
    dropdownCssClass: "country-drop",
    dropdownParent: $(this).next('.country-modal')
  });
});
$(".phone-select").each(function(){
  $(this).select2({
    minimumResultsForSearch: -1,
    containerCssClass: "phone-select",
    dropdownCssClass: "phone-drop",
    dropdownParent: $(this).next('.phone-modal')
  });
});
...
// Auto height
// $(document).ready(function(){
//   $('[class^="heightDefault"]').each(function(){
//     var curentHeight = $(this).attr('class').replace(/^\+ ?(\d+).*$/, '$1');
...
// if( $("div").hasClass('.swiper-container')){
  ...
  function initSwiper() {
      ...
      if(screenWidth < 1025 && mySwiper == undefined) {
...
$(".swiper-container").each(function(){
  var colSlides = $(this).find('.swiper-wrapper .swiper-slide').length;
  
  if(colSlides > 6){
    $(this).find('.swiper-button-next, .swiper-button-prev, swiper-scrollbar--style').show();
    mySwiper = new Swiper($(this), {
      // spaceBetween: 0,
      freeMode: true,
      slidesPerView :'auto',
    ...
    $(this).find('.swiper-button-next, .swiper-button-prev, swiper-scrollbar--style').hide();
  }
});
...
// $(".rangel").ionRangeSlider({
//   keyboard: true,
//   min: 1,
//   max: colSlides,
//   extra_classes: "team__pagination-slider",
//   hide_from_to: true,
//   hide_min_max: true,
//   onChange: function (data) {
//     var page = data.from*1;
//     console.log(data.from)
//     // owl.trigger('to.owl', page);

//     // const pageText = $(".team__pagination-pages-from")[0];
//     // $(pageText).text(data.from);
//   }
// });
// });
...
var menuOpen = function(){
  $('.mobile-burger').click(function(e){
    ...
    $('.overlay-menu--close').toggleClass('is-active');
    $('.wrap-menu').toggleClass('is-active');
    $('body').addClass('no-overflow');
  });
  $(document).mouseup(function (e) {
    ...
    if (container.has(e.target).length === 0){
      ...
      $('.overlay-menu--close').removeClass('is-active');
      // $('body').removeClass('no-overflow');
    }
  });
};
...
$('#no-overflow').click(function(e){
  $('body').removeClass('no-overflow');
});
$('.get-touch--link').click(function(e){
  $('.wrap-menu').removeClass('is-active');
  $('.mobile-burger').removeClass('is-active');
  $('#sidebar').removeClass('overlay');
  $('.overlay-menu--close').removeClass('is-active');
...
var stiky = function(){
  var div = $('.logo-sidebar');
  var start = $(div).offset().top;

  // $.event.add(window, "scroll", function() {
  //   var p = $(window).scrollTop();
  //   console.log(p)
  //   $(div).css('position',((p)>70) ? 'fixed' : 'absolute');
  //   $(div).css('top',((p)>70) ? '-70px' : '0px');

  //   if((p)>70){
  //     div.addClass("stiky");
  //   } else {
  //     div.removeClass("stiky");
  //   }
  // });

  var lastScrollTop = 0;
  var startScrollTop = 30;
  $(window).scroll(function(event){
    var st = $(this).scrollTop();
    if (st > startScrollTop){
     // $(div).css('top','-70px');
     $(div).css('transform','translate(0, -69px)');
     // $(div).css('position','fixed');
   }
   if(st < lastScrollTop) {
    // $(div).css('top','0px');
    $(div).css('transform','translate(0, 0px)');
    // $(div).css('position','fixed');
   }
   lastScrollTop = st;
 });
  ...
  jQuery("#sendform").validate({
    submitHandler: function(form){
      ...
      xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
          if(xhr.status == 200) {
            $('.succesfull-wrapper').addClass('is-active');
            $('.succesfull-wrapper .overlay-bg').addClass('is-active');
            $('.succesfull-post').addClass('is-active');
            $("#sendform").trigger('reset');
            initTimer();
            $('.form-perspective .overlay-bg').removeClass('is-active');
...
$('.succesfull-wrapper .overlay-bg, .succesfull-post .back-done').on('click', function(){
  $('.succesfull-wrapper').removeClass('is-active');
});
...
//   $(this).next(".MultiFile-wrap").find("input#trrr").click()
// });

// $('.add-load').each(function(){
//   $(".upload-label").on('click', function(){
...
$(".upload-label").on('click', function(){
  ...
  //   var options = {
  //     zoom: 5,
  //     center: {lat: 51.491686, lng: -0.038784},
  //     styles: [
  //     {
  //       "stylers": [
  //       {
  ...
  //     },
  //     {
  //       coordinates: {lat: 43.30257, lng: 5.37583},
  //       image: 'img/pin-map.png',
  //       info: '<p>Marselle</p>'
  //     }
  //   ];
