...
$(".city-select").select2({
	...
	dropdownCssClass: "city-drop",
	containerCssClass: "city-select",
});

// $('.city-select').on('change', function(){
// 	console.log($('.city-select option:selected').attr('data-coor1'))
// });
...
function init() {
	...
	var createContClick = function(slice){
		...
		function showPrewievBlock(){
			cloneLists = slice.clone()
			cloneLists.appendTo(listAppend.find('#dealer-list-wiev--main'))
   ...
   //  	e.get('target').options.set('iconLayout', 'default#imageWithContent');
    ...
    function createDealerList (group) {
    	...
    	function onCheckData(data){
    		...
    		if(data == undefined){
    			...
    			for(var i = 0; i<data.length; i++) {
    				images += '<a href="'+data[i]+'"><img src="'+data[i]+'"/></a>';
    			}
    	...
    	myMap.panTo(coords, {
    		...
    		if($(this).hasClass('is-active')){
    			...
    			createContClick(slice.parent('li'));
	...
	// 	if (activePlacemark) {
	// 		activePlacemark.options.set('iconLayout', 'default#imageWithContent');
	// 		activePlacemark.options.set('iconImageSize', [30, 30]);
	// 		activePlacemark.options.set('iconImageOffset',  [-15, -15]);
	// 		activePlacemark.options.set('iconImageHref', 'img/icon/icon-location.svg');
	// 		activePlacemark = undefined;
	// 	} else {
	// 		e.get('target').options.set('iconLayout', 'default#imageWithContent');
	// 		e.get('target').options.set('iconImageSize', [30, 30]);
	// 		e.get('target').options.set('iconImageOffset',  [-15, -15]);
   ...
   //  		e.get('target').options.set('iconImageSize', [30, 30]);
   //  		e.get('target').options.set('iconImageOffset',  [-15, -15]);
   //  		e.get('target').options.set('iconImageHref', 'img/icon/icon-location.svg');
	  //   // iconImageHref: homeUrl+'/img/icon/icon-location.svg',
	  // });
    }

    $('.city-select').on('change', function(){
    	var coords = $('.city-select option:selected').attr('data-coor1').split(", ").map(Number);
    	myMap.panTo(coords, {
    		flying: 1
    	});
    });
    ...
    $('.close-prewiev').click(function(e){
    	...
    	$('#dealer-list-wiev--main li').remove();
    	$('.dealer-list .links-map').removeClass('is-active');
    });
...
function startGal(){
	...
	$('#dealer-list-wiev--main .gallery-wrapper').magnificPopup({
		delegate: 'a',
		type: 'image',
